class AttendancesController < ApplicationController
  ...
  def edit_log
    if params[:worked_on].present?
      @approval_change_attendance_requests = Attendance.order(:worked_on).where('worked_on LIKE ?', "#{params[:worked_on]}%").where(user_id: current_user, edit_attendance_request_status: "承認")
